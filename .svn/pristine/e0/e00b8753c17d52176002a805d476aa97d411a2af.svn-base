...
public class InputTag extends org.springframework.web.servlet.tags.form.InputTag {
    ...
    protected void writeValue(TagWriter tagWriter) throws JspException {
        ...
        if (fmt == null) {
            ...
            value = fmt.format(getBindStatus().getActualValue());
        }

        String sValue = getDisplayString(value, getPropertyEditor());
        String type = hasDynamicTypeAttribute() ? (String) getDynamicAttributes().get("type") : getType();
        tagWriter.writeAttribute("value", processFieldValue(getName(), sValue, type));
    }
