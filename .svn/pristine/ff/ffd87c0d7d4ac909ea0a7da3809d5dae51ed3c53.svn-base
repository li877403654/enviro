...
public class StrFuncs {
	...
	public static String join(int[] arr, char sep) {
		if(arr == null || arr.length == 0) {
			return "";
		}
		StringBuilder sb = new StringBuilder();
...
//		return "%" + s.replace("_", "[_]").replace("%", "[%]") + "%";
    ...
    public static String trimStart(String str, char... charsToTrim) {
        char[] chars = charsToTrim;
        if (chars.length == 0) {
            chars = new char[] { '\r', '\n', '\t', ' ' };
        }
	...
	public static String trim(String str, char... charsToTrim) {
        str = trimStart(str, charsToTrim);
        str = trimEnd(str, charsToTrim);
		return str;
	}
